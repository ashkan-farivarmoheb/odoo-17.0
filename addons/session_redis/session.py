...
class RedisSessionStore(SessionStore):
    ...
    def __init__(
        ...
        if prefix:
         ...
         # Adding debug logs
        _logger.debug(
            "RedisSessionStore initialized with prefix: %s, expiration: %s, anon_expiration: %s",
            self.prefix,
            self.expiration,
    ...
    def build_key(self, sid):
        _logger.debug("build key triggered %s %s",self.prefix, sid)
        return "%s%s" % (self.prefix, sid)

    def save(self, session):
        _logger.debug("save key triggered")
    ...
    def delete(self, session):
        _logger.debug("delete  triggered")
    ...
    def get(self, sid):
        _logger.debug("get  triggered %s",sid)
    ...
    def vacuum(self):
        _logger.debug("vacucun  triggered")
