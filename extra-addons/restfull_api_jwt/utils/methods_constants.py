...
def check_and_remove_country_code_of_saudi_arabia(phone_number):
    ...
    if re.match(pattern, phone_number):
        for key in keys:
            if phone_number.startswith(key):
                return phone_number[len(key):]  # Directly return the stripped number
        return phone_number  # Return the original number if no key matched
    return phone_number  # Return the original number if it doesn't match the pattern
