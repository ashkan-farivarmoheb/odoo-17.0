    ...
    def signUp(self):
        ...
        if not any(field not in args.keys() for field in required_fields):
            ...
            confirmToken = create_token(
                validatorConfirmAuth, expiresIn=refreshTokenExpiresIn, partner_id=user_sudo.partner_id.id)
            request.env[ModelsName.authUsersTokens].sudo().create(
                UsersToken.toMap(token=confirmToken, refresh_token=None,
                                 type=nameConfirmCodeToken, user_id=user_sudo.id)
            )
        ...
        if 'login' in args and 'password' in args:
            ...
            if user_sudo:
                ...
                accessToken = create_token(
                    validatorPortalAuth, expiresIn=accessTokenExpiresIn, partner_id=user_sudo.partner_id.id)
                refreshToken = create_token(
                    validatorRefreshAuth, expiresIn=refreshTokenExpiresIn, partner_id=user_sudo.partner_id.id)
                request.env[ModelsName.authUsersTokens].sudo().create(
                    UsersToken.toMap(token=accessToken, refresh_token=refreshToken,
                                     type=nameAccessToken, user_id=user_sudo.id)
                )
            ...
            if user_sudo:
                if user_sudo.confirmation_code == confirmation_code:
                    ...
                    confirmResetToken = create_token(
                        validatorResetAuth, expiresIn=confirmResetExpiresIn, partner_id=user_sudo.partner_id.id)
                    request.env[ModelsName.authUsersTokens].sudo().create(
                        UsersToken.toMap(
                            token=confirmResetToken, refresh_token=None, type=nameResetToken, user_id=user_sudo.id)
                    )
                else:
                    raise ParamsErrorException(
            ...
            if user_sudo:
                if not user_sudo.is_complete:
                    if user_sudo.confirmation_code == confirmation_code:
                        ...
                        request.env[ModelsName.authUsersTokens].sudo().create(
                            UsersToken.toMap(
                                token=accessToken, refresh_token=refreshToken, type=nameAccessToken, user_id=user_sudo.id)
                        )
                        user_responce = UserAuthResponce(
                            id=user_sudo.id,
                            name=check_data(user_sudo.name),
