...
class Serializer(object):
    ...
    def data(self):
        ...
        _logger.debug("OdooAPI  Serializer %s %s %s",self._record,self.many,parsed_restql_query)
    ...
    def build_flat_field(cls, rec, field_name):
        ...
        _logger.debug("OdooAPI  build_flat_field %s:%s ",field_name,field_type)
        _logger.debug("OdooAPI  build_flat_field rec %s ",rec)
        
        try:
            field_value = rec[field_name]
            result= {field_name: field_value}
        except KeyError:
            # Handle the case where the field does not exist in the record
            field_value = getattr(rec, field_name, None)

            msg = "'%s' field does not exist in the record" % field_name
            # _logger.debug("OdooAPI  build_flat_field Exception %s:%s",field_name,rec.fields_get(field_name).get(field_name))
            # _logger.debug("OdooAPI  build_flat_field Exception rec %s:%s",field_name,field_value)
            _logger.debug("OdooAPI  build_flat_field Exception %s ",msg)

        except Exception as e:
            # Handle other exceptions that might occur when accessing the field
            msg = "Error accessing '%s' field: %s" % (field_name, str(e))
            _logger.debug("OdooAPI  build_flat_field Exception %s ",msg)
        else:
            if field_type in ['one2many', 'many2many']:
                # _logger.debug("OdooAPI  build_flat_field many2many  %s:%s",field_name,rec.fields_get(field_name).get(field_name))
                # _logger.debug("OdooAPI  build_flat_field many2many rec  %s:%s",rec,rec[field_name])
                return {
    ...
    def serialize(cls, rec, parsed_query):
        ...
        if parsed_query["exclude"]:
            ...
            for field in flat_fields:
                ...
                if hasattr(flat_field, '__iter__') and not isinstance(flat_field, (str, bytes)):
                    data.update(flat_field)
                else:
                    # Handle the case where flat_field is not iterable
                    msg = "'%s' field is not iterable or not supported" % field
                    _logger.debug("OdooAPI  Serializer %s",msg)
                # data.update(flat_field)
