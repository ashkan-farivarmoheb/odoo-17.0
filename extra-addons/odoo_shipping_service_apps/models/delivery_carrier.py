...
class DeliveryCarrier(models.Model):
    ...
    @api.model
    # Method called by Every Delivery Carrier with Fields as Argument ,which should a list for return their credentials as Dictionary.
    def wk_get_carrier_settings(self, Fields):
        res = self.read(Fields)[0]
        lst = []
        for key in res:
            if key != 'prod_environment' and not res[key]:
                lst.append(key)
        if lst:
            raise UserError(
                f"The following fields are missing for {self.name}:" + '\n' + '\n'.join(lst))
        return res
        ...
        if qty_capacity and qty_capacity < max_qty:
            ...
            if package_count:
                ...
                multi_pckg_qty_capacity.update(dict(
                    weight=round(product_weight*qty_capacity, 3),
                    full_capacity=True,
            ...
            if product_qty % qty_capacity:
                ...
                single_pckg_qty_capacity.update(dict(
                    weight=round(product_weight*(product_qty %
                                 qty_capacity), 3),
                    full_capacity=False,
            ...
            if package_count:
                ...
                multi_pckg_max_qty.update(dict(
                    weight=round(product_weight*max_qty, 3),
                    full_capacity=True,
            ...
            if product_qty % max_qty:
                ...
                single_pckg_max_qty.update(dict(
                    weight=round(product_weight*(product_qty % max_qty), 3),
                    full_capacity=False,
    ...
    def wk_get_packaging_id(self, product_id=None, package_id=None):
        ...
        if product_id:
            packaging_ids = product_id.wk_packaging_ids.filtered(
                lambda pck_id: pck_id.package_type_id.package_carrier_type == self.delivery_type)
    ...
    def wk_merge_half_package(self, items):
        ...
        for item in filter(lambda item: not item.get('full_capacity'), items):
            ...
            pac_id = self.env['stock.package.type'].browse(packaging_id)
            if type(packaging_id) == int:
                # packaging_id = self.env['product.packaging'].browse(packaging_id)
                packaging_id = self.env['product.packaging'].search(
                    [('package_type_id', '=', pac_id.id)])
    ...
    def _get_api_weight(self, shipping_weight):
        q = self._get_default_uom()._compute_quantity(
            1, self.uom_id)
        _logger.debug(
            "  order _compute_quantity q:%s uom_id:%s shipping_weight:%s", q, self.uom_id, shipping_weight)
    ...
    def convert_shipment_price(self, data):
        ...
        if currency_id and currency:
            if currency_id.name != currency:
                ...
                actual_price = currency._convert(price, currency_id)
        return actual_price
    ...
    def wk_validate_object_fields(self, obj, fields):
        """
        Arguments: Function takes in an object and the 'required fields' of the object as a List
                Returns: Function raises error if any field is missing.
        """
        obj_fields_values = obj.read(fields)[0]
        missing_required_fields = []
        for key in obj_fields_values:
            if not obj_fields_values[key]:
                missing_required_fields.append(key)
        if not missing_required_fields:
            return True
        else:
            raise UserError(
                f"The following required fields are missing for {obj.name if hasattr(obj, 'name') else obj}:\n" + "\n".join(missing_required_fields))
